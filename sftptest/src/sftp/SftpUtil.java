 ...
 * 2. @since version-0.3
 * 方法出现异常的时候，<del>会关闭sftp连接(但是不会关闭session和channel)</del>(del @ version
 * 0.31)，异常会抛出<br>
 *
 * @author Leon Lee
 */
public class SftpUtil {
	...
	public static ChannelSftp getSftpConnect(final String host, final int port, final String username,
		...
		String key = host + "_" + port + "_" + username + "_" + password;
		if (SFTP_CHANNEL_POOL.containsKey(key)) {
			JSch jsch = new JSch();
	 ...
	 * @throws Exception
	 */
	public static ChannelSftp getSftpConnect4Key(final String host, final int port, final String username,
			final String privateKey, final String passphrase) throws Exception {
		Session sshSession = null;
		Channel channel = null;
		ChannelSftp sftp = null;
		String key = host + "_" + port + "_" + username + "_" + privateKey + "_" + passphrase;
		if (!SFTP_CHANNEL_POOL.containsKey(key)) {
			JSch jsch = new JSch();
			// 设置密钥和密码
